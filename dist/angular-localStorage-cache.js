...
(function (SpeedShifter) {
    (function (Services) {
        Services.LocalStorageProvider = [
            '$log', '$window', function ($log, $window) {
                var _localStorage = ((typeof $window.localStorage === 'undefined') ? undefined : $window.localStorage), _localStorage_supported = !(typeof _localStorage === 'undefined' || typeof JSON === 'undefined');
                ...
                this.name = '';
                this.config = {};
                var provider = this, serviceProvider = this, storage = {
                    get: function (key) {
                        if (!_localStorage_supported)
                            return null;
                ...
                this.$get = [function () {
                        ...
                        me.get = function (valName, propertyName) {
                            var storageValName = provider.name + "." + valName, propertyValName = storageValName, config = provider.config[valName];
                        ...
                        me.set = function (valName, propertyName, val, updated) {
                            var storageValName = provider.name + "." + valName, propertyValName = storageValName, config = provider.config[valName];
                            propertyName && (propertyValName += "." + propertyName);
                            try  {
                                if (config) {
                                    if (config.user_dependent)
                                        storage.set(storageValName + '.uid', userId);
                                ...
                                if (config && config.multiple) {
                                    ...
                                    me.remove(valName, null, limit);
                                }
                            }
                        };
                        me.remove = function (valName, propertyName, limit) {
                            var storageValName = provider.name + "." + valName, propertyValName = storageValName, config = provider.config[valName];
                            propertyName && (propertyValName += "." + propertyName);
                            if (config) {
                                if (config.multiple && !propertyName) {
                                    ...
                                    if (config.limit)
                                        storage.set(storageValName + '.count', Math.max(storage.get(storageValName + '.count') - c, 0));
                                } else {
                                    storageValName += (propertyName ? "." + propertyName : "");
                                    storage.remove(propertyValName);
                                    if (config.limit)
                                        storage.set(storageValName + '.count', Math.max(storage.get(storageValName + '.count') - 1, 0));
                                    if (config.user_dependent)
                                        storage.remove(storageValName + '.uid');
                                    if (config.expires)
                                        storage.remove(propertyValName + '.updated');
                                }
                            } else {
                                storage.remove(propertyValName);
                                if (config.limit)
                        ...
                        me.setUserId = function (id) {
                            if (angular.isUndefined(id) || id === '' || id === null || id !== id)
                                return;
                            ...
                            userId = "" + id_;
                            var confs = provider.config, storageValName, valName;
                ...
                serviceProvider.addStoreConfig = function (config) {
